...
file_path = '{0}/sub4_{1}.txt'.format(path_dir, arg)
...
def Wordcount_Thai(text):
    ...
    with open('/home/tengmo/crawler_to_server_set_time/crawler/output_thai/git/word-parser-noPartOfSpeech/wordcut4/input.txt' , 'w') as out:
        out.write(text)
    #print(datainput)
    command= os.popen("bash /home/tengmo/crawler_to_server_set_time/crawler/dynamic_section/soucecode/run_java4.bash")
    datainput = command.read()
    #print(datainput)
    with open('/home/tengmo/crawler_to_server_set_time/crawler/output_thai/git/word-parser-noPartOfSpeech/wordcut4/output.csv', 'r') as csvfile:
...
if (os.path.exists('{0}/json/dict4_{1}.json'.format(path_dir, arg))):
    print "dict2_%s already exists"%arg
    sys.exit()
####################################-Rotate-###############################################
cnt_begin = 0
cnt_end = 1
...
###########################-SUMARY-#########################################################
#print json_dict
with open('{0}/json/dict4_{1}.json'.format(path_dir, arg), 'w') as outfile:
    json.dump(json_dict, outfile, sort_keys = True, indent = 4,encoding='utf8')
outfile.close()
'''
print "#####################################################################################"
print "SUMARY"
