...
import json
clus = eval(open("/home/tengmo/crawler_to_server_set_time/crawler/source_code_python2.7/cluster/center.txt").read())
vector = eval(open("/home/tengmo/crawler_to_server_set_time/crawler/source_code_python2.7/cluster/name.txt").read())
...
for k, v in result.iteritems():
    ...
    for x in range(0, len(v)):
        #print v[x]
        print v[x][0:21]
        score =  cos_sim(clus["cen"][k],v[x][0:21])
        final_result[k][v[x][-1]] = score
    final_result[k] = sorted(final_result[k].items(), key=lambda x:x[1],reverse=True)


#print final_result
with open('/home/tengmo/crawler_to_server_set_time/crawler/source_code_python2.7/cluster/class.json', 'w') as outfile:
